...
BasicGame.Game.prototype = {
	...
	create: function () {
		...
		//Create the lives group, set the physcics and the boundaries
	...
	//function executed during plaing the game to create a UFO
	createUfo: function () {
		...
		if (random === 0) {
			//Generating random postion in the X Axis
			var randomX = this.rnd.intergerInRange(0, this.world.width - 150);
			//Creating a ufo from the ufos group setting physics
	...
	createLife: function () {
		...
		if (random === 0) {
			//Generating random position in the X Acis
			var randomX = this.rnd.intergerInRange(0, this.world.width - 150);
	...
	//Generate bullet and postion in the x axis, set the velocity and play the audio
	fireBullet: function () {
	...
	collisionDetection: function () {
		...
		this.physics.arcade.overlap(bullet, ufo, this.destoryUfo, null, this);
	},

	//function exexcuted if there is collision between player and ufo. UFO is destroyed, aniimation & sound, reduce lifeTotal
	...
	//function executed if there is collision between ufo an bullet. UFO is destroyed, animation & sound, increase score
...
//function is executed when the game ends. Stops Ship, Kills all objects, stop timer, Display Restart button
gameOver: function () {
	ship.body.velocity.x = 0;
